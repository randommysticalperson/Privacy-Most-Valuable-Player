...
import { type ForumThread, type ThreadCategory, type PrivacyBadge } from "@/lib/forumStore";
...
export default function NewThreadModal({ open, onClose, onCreated }: NewThreadModalProps) {
  ...
  const handleCreate = async () => {
    if (!title.trim()) {
      toast.error(lang === "zh" ? "請輸入標題" : "Please enter a title");
      return;
    }
    setIsPosting(true);
    try {
      ...
      if (useEncryption && content) {
        ...
        toast.success(
          (lang === "zh" ? "討論串已加密，請保存您的金鑰：" : "Thread encrypted. Save your key: ") + exportedKey.slice(0, 16) + "...",
          { duration: 8000 }
        );
      }
      ...
      toast.error(lang === "zh" ? "建立討論串失敗" : "Failed to create thread");
    } finally {
      setIsPosting(false);
    }
  };
      ...
      <DialogContent className="bg-[oklch(0.1_0.01_265)] border-border max-w-lg">
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">{t("threadTitle")} *</label>
            <Input
              value={title}
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">
              {lang === "zh" ? "開筆內容（選填）" : "Opening content (optional)"}
            </label>
            <Textarea
              value={content}
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">
              {lang === "zh" ? "標籤（逗號分隔）" : "Tags (comma separated)"}
            </label>
            <Input
              value={tags}
              onChange={e => setTags(e.target.value)}
              placeholder={lang === "zh" ? "zkp, 身份, 隱私" : "zkp, identity, privacy"}
              className="bg-[oklch(0.14_0.015_265/0.5)] border-border text-sm"
            />
          </div>
          ...
          <div className="space-y-2 p-3 rounded-lg bg-[oklch(1_0_0/0.03)] border border-border">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <div
                ...
                className={`w-8 h-4 rounded-full transition-all relative flex items-center shrink-0 ${useEncryption ? "bg-[oklch(0.7_0.17_162)]" : "bg-[oklch(1_0_0/0.1)]"}`}
              >
              ...
              <div>
                <p className="text-xs text-foreground flex items-center gap-1">
                  ...
                  {lang === "zh" ? "端對端加密（AES-GCM-256）" : "End-to-End Encryption (AES-GCM-256)"}
                </p>
                <p className="text-[10px] text-muted-foreground">
                  {lang === "zh" ? "伺服器僅儲存密文" : "Server stores only ciphertext"}
                </p>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <div
                ...
                className={`w-8 h-4 rounded-full transition-all relative flex items-center shrink-0 ${useZkp ? "bg-[oklch(0.75_0.18_75)]" : "bg-[oklch(1_0_0/0.1)]"}`}
              >
              ...
              <div>
                <p className="text-xs text-foreground flex items-center gap-1">
                  ...
                  {lang === "zh" ? "ZKP 身份徽章（Semaphore）" : "ZKP Identity Badge (Semaphore)"}
                </p>
                <p className="text-[10px] text-muted-foreground">
                  {lang === "zh" ? "證明群組成員身份，不暴露真實身份" : "Prove group membership without revealing identity"}
                </p>
              </div>
            </label>
          </div>
